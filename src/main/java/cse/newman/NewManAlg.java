...
//
//		// 保存已经计算的节点对，防止重复计算，因为论文中有tips
//		Set<String> calcuAi = new HashSet<String>();
//		double ai = 0;
//		for (int i : clusterI_id) {
//			for (int j : clusterI_set) {
//				if (calcuAi.contains(i + "" + j)
//						|| calcuAi.contains(j + "" + i)) {
//					continue;
//				} else {
//					calcuAi.add(i + "" + j);
//					calcuAi.add(j + "" + i);
...
//	}
	 ...
	 * @return
	 */
	public double deltaQ(List<String> clusterI, List<String> clusterJ) {
		// 首先获取两个cluster中的所有ID
		Set<String> clusterI_id = new HashSet<String>();
		clusterI_id.addAll(clusterI);

		Set<String> clusterJ_id = new HashSet<String>();
		clusterJ_id.addAll(clusterJ);
		...
		// 循环遍历两个集合，生成最终的eij
		for (String i : clusterI_id) {
			for (String j : clusterJ_id) {
				/**
				 * 若已经计算过了，则忽略
				 */
				if (innerNodeSet.contains(i + "" + j)
		...
		// 生成最终的ai
		// 保存与clusterI相邻的所有节点
		Set<String> clusterI_set = new HashSet<String>();
		for (String nodeId : clusterI_id) {
			// System.out.println(community.getNodeList());
			Node tmp = community.getNodeById(nodeId);
			Set<String> neiborId = tmp.getAllNeibor();
		...
		// 保存与clusterJ相邻的所有节点
		Set<String> clusterJ_set = new HashSet<String>();
		// for (Node node : clusterJ) {
		for (String nodeId : clusterJ_id) {
			Set<String> neiborId = community.getNodeById(nodeId)
					.getAllNeibor();
		...
		// 计算aj
		double aj = 0;
		for (String i : clusterJ_id) {
			for (String j : clusterJ_set) {
	...
	public static void main(String[] args) {
		...
		List<String> clusterI = new ArrayList<String>();
		clusterI.add("2");
		// clusterI.add(4);
		List<String> clusterJ = new ArrayList<String>();
		clusterJ.add("4");
